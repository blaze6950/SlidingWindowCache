...
namespace Intervals.NET.Caching.SlidingWindow.Benchmarks.Benchmarks;
...
///    - Submit BurstSize sequential requests (await each - SlidingWindowCache is single consumer)
...
public class ExecutionStrategyBenchmarks
{
    ...
    /// <list type="number">
    /// <item><description>Submit BurstSize requests sequentially (await each - SlidingWindowCache is single consumer)</description></item>
    ...
    {
        // Submit all requests sequentially (NOT Task.WhenAll - SlidingWindowCache is single consumer)
