...
/// <summary>
/// Task-based work scheduler that serializes work item execution using task continuation chaining.
...
/// </list>
/// <para>See also: <see cref="ChannelBasedWorkScheduler{TWorkItem}"/> for the bounded alternative with backpressure.</para>
/// </remarks>
internal sealed class TaskBasedWorkScheduler<TWorkItem> : WorkSchedulerBase<TWorkItem>
    where TWorkItem : class, ISchedulableWorkItem
{
    ...
    /// <summary>
    /// Initializes a new instance of <see cref="TaskBasedWorkScheduler{TWorkItem}"/>.
    /// </summary>
    /// <param name="executor">
    ...
    public override ValueTask PublishWorkItemAsync(TWorkItem workItem, CancellationToken loopCancellationToken)
    {
        if (IsDisposed)
        {
            throw new ObjectDisposedException(
                nameof(TaskBasedWorkScheduler<TWorkItem>),
