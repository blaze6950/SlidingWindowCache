...
Converted tests:
- `Invariant_SWC_A_2a_UserRequestCancelsRebalance`
- `Invariant_SWC_C_1_AtMostOneActiveIntent`
- `Invariant_SWC_F_1_G_4_RebalanceCancellationBehavior`
...
Converted tests:
- `Invariant_SWC_A_12_UserPathNeverMutatesCache` (3 scenarios × 2 storage = 6 test cases)
- `Invariant_SWC_F_2a_RebalanceNormalizesCache`
...
- **Location**: `src/Intervals.NET.Caching/Public/Instrumentation/`
- **Files**:
...
- **Location**: `tests/Intervals.NET.Caching.Invariants.Tests/TestInfrastructure/`
- **Files Created**:
...
- SWC.A.1: User Path and Rebalance Execution never write concurrently (User Path doesn't write)
- SWC.A.12: User Path MUST NOT mutate cache (enforced by removing Rematerialize calls)
- SWC.F.2: Rebalance Execution is ONLY writer (enforced by internal setters)
- SWC.C.8e/f: Intent contains delivered data (enforced by PublishIntent signature)

## Usage

```bash
# Run all invariant tests
dotnet test tests/Intervals.NET.Caching.SlidingWindow.Invariants.Tests/Intervals.NET.Caching.SlidingWindow.Invariants.Tests.csproj --configuration Debug
...
dotnet test --filter "FullyQualifiedName~Invariant_SWC_D"
```
...
1. **Policy-Based Skip** (Invariants SWC.D.3 / SWC.D.5)
...
2. **Optimization-Based Skip** (Invariant SWC.D.4)
...
See `docs/sliding-window/storage-strategies.md` for detailed documentation.
...
- `docs/sliding-window/invariants.md` - Complete invariant documentation
- `docs/sliding-window/state-machine.md` - State transitions and mutation authority
- `docs/sliding-window/actors.md` - Actor responsibilities and component mapping
- `docs/sliding-window/architecture.md` - Concurrency model and single-writer rule
...
**See**: [Diagnostics Guide](../../docs/sliding-window/diagnostics.md) for comprehensive diagnostic API reference
