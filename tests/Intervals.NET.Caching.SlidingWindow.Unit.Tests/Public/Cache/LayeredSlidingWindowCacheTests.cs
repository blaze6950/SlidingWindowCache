...
/// <summary>
/// Unit tests for <see cref="LayeredRangeCache{TRange,TData,TDomain}"/>.
/// Validates delegation to the outermost layer for data operations, correct layer count,
/// and disposal ordering. Uses mocked <see cref="ISlidingWindowCache{TRange,TData,TDomain}"/> instances
...
public sealed class LayeredSlidingWindowCacheTests
{
    ...
    private static Mock<ISlidingWindowCache<int, int, IntegerFixedStepDomain>> CreateLayerMock() => new(MockBehavior.Strict);

    private static LayeredRangeCache<int, int, IntegerFixedStepDomain> CreateLayeredCache(
        params ISlidingWindowCache<int, int, IntegerFixedStepDomain>[] layers)
    {
        ...
        return CreateLayeredCacheFromList(layers.ToList<IRangeCache<int, int, IntegerFixedStepDomain>>());
    }

    private static LayeredRangeCache<int, int, IntegerFixedStepDomain> CreateLayeredCacheFromList(
        IReadOnlyList<IRangeCache<int, int, IntegerFixedStepDomain>> layers)
    {
        // Instantiate via the internal constructor using the test project's InternalsVisibleTo access.
        return new LayeredRangeCache<int, int, IntegerFixedStepDomain>(layers);
    }
    ...
    public async Task GetDataAsync_PropagatesCancellationToken()
    {
        ...
        outerLayer.Setup(c => c.GetDataAsync(range, It.IsAny<CancellationToken>()))
            .Returns<Range<int>, CancellationToken>((_, ct) =>
            {
                capturedToken = ct;
    ...
    #region IRangeCache Interface Tests

    [Fact]
    public void LayeredWindowCache_ImplementsIRangeCache()
    {
        // ARRANGE
        var layer = CreateLayerMock();
        layer.Setup(c => c.DisposeAsync()).Returns(ValueTask.CompletedTask);
