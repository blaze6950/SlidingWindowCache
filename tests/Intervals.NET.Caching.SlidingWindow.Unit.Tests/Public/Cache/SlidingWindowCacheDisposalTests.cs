...
public class SlidingWindowCacheDisposalTests
{
    ...
    private static SlidingWindowCache<int, int, IntegerFixedStepDomain> CreateCache()
    {
        ...
        return new SlidingWindowCache<int, int, IntegerFixedStepDomain>(dataSource, domain, options);
    }
    ...
    public async Task DisposeAsync_WithActiveBackgroundRebalance_WaitsForCompletion()
    {
        // ARRANGE
        var cache = CreateCache();
        var range1 = Factories.Range.Closed<int>(0, 10);
        var range2 = Factories.Range.Closed<int>(100, 110);
    ...
    public async Task DisposeAsync_WithHighConcurrency_HandlesGracefully()
    {
        ...
        var tasks = Enumerable.Range(0, 50)
            .Select(i => cache.GetDataAsync(
                Factories.Range.Closed<int>(i * 10, i * 10 + 10),
