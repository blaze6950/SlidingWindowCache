...
namespace Intervals.NET.Caching.SlidingWindow.Unit.Tests.Public.Extensions;

/// <summary>
/// Unit tests for <see cref="SlidingWindowCacheConsistencyExtensions.GetDataAndWaitForIdleAsync{TRange,TData,TDomain}"/>
/// and <see cref="SlidingWindowCacheConsistencyExtensions.GetDataAndWaitOnMissAsync{TRange,TData,TDomain}"/>.
...
public sealed class SlidingWindowCacheConsistencyExtensionsTests
{
    ...
    private static Mock<ISlidingWindowCache<int, int, IntegerFixedStepDomain>> CreateMock() => new(MockBehavior.Strict);

    private static Range<int> CreateRange(int start, int end)
