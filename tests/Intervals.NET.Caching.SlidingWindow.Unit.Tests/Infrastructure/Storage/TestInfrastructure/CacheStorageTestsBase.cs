...
using static Intervals.NET.Caching.SlidingWindow.Unit.Tests.Infrastructure.Storage.TestInfrastructure.StorageTestHelpers;
...
public abstract class CacheStorageTestsBase
{
    ...
    [Fact]
    public void Invariant_SWC_B_1_DataLengthMatchesRangeSize_AfterMultipleRematerializations()
    {
        // ARRANGE
        var domain = CreateFixedStepDomain();
        var storage = CreateStorage(domain);
    ...
    [Fact]
    public void Invariant_SWC_B_1_PartialReads_ConsistentWithStoredRange()
    {
        // ARRANGE
        var domain = CreateFixedStepDomain();
        var storage = CreateStorage(domain);
