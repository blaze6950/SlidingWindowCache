...
namespace Intervals.NET.Caching.SlidingWindow.Tests.Infrastructure.Helpers;
...
public static class TestHelpers
{
    ...
    /// This is the standard range type used throughout the SlidingWindowCache system.
    /// </summary>
    ...
    public static Range<int> CreateRange(int start, int end) => Factories.Range.Closed<int>(start, end);

    /// <summary>
    /// Creates default cache options for testing.
    /// </summary>
    public static SlidingWindowCacheOptions CreateDefaultOptions(
    ...
    /// <summary>
    /// Creates a SlidingWindowCache instance with the specified options.
    /// </summary>
    public static SlidingWindowCache<int, int, IntegerFixedStepDomain> CreateCache(
    ...
    /// <summary>
    /// Creates a SlidingWindowCache instance backed by a <see cref="SpyDataSource"/>.
    ...
    /// <summary>
    /// Creates a SlidingWindowCache with default options and returns both cache and mock data source.
    /// </summary>
    public static (SlidingWindowCache<int, int, IntegerFixedStepDomain> cache, Mock<IDataSource<int, int>> mock)
        CreateCacheWithDefaults(
            IntegerFixedStepDomain domain,
            EventCounterCacheDiagnostics cacheDiagnostics,
            SlidingWindowCacheOptions? options = null,
            TimeSpan? fetchDelay = null
        )
    {
    ...
    /// <summary>
    /// Asserts that rebalance was skipped because DesiredCacheRange equals CurrentCacheRange (Stage 4 / SWC.D.4).
    /// </summary>
    /// <param name="cacheDiagnostics">The diagnostics instance to check.</param>
