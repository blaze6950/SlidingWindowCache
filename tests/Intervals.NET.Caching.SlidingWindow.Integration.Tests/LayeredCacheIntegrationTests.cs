...
/// <see cref="LayeredRangeCacheBuilder{TRange,TData,TDomain}"/>,
/// <see cref="LayeredRangeCache{TRange,TData,TDomain}"/>, and
...
public sealed class LayeredCacheIntegrationTests
{
    ...
    private static SlidingWindowCacheOptions DeepLayerOptions() => new(
        leftCacheSize: 5.0,
        rightCacheSize: 5.0,
    ...
    private static SlidingWindowCacheOptions MidLayerOptions() => new(
        leftCacheSize: 2.0,
        rightCacheSize: 2.0,
        readMode: UserCacheReadMode.CopyOnRead,
        leftThreshold: 0.3,
    ...
    private static SlidingWindowCacheOptions UserLayerOptions() => new(
        leftCacheSize: 0.5,
        rightCacheSize: 0.5,
    ...
    public async Task TwoLayerCache_GetDataAndWaitForIdleAsync_SubsequentRequestIsFullHit()
    {
        ...
        var subRange = Factories.Range.Closed<int>(402, 408);
    ...
    public async Task WindowCacheDataSourceAdapter_UsedAsDataSource_PropagatesDataCorrectly()
    {
        ...
        var deepCache = new SlidingWindowCache<int, int, IntegerFixedStepDomain>(
            realSource, Domain, DeepLayerOptions());

        await using var _ = deepCache;

        var adapter = new RangeCacheDataSourceAdapter<int, int, IntegerFixedStepDomain>(deepCache);
        var userCache = new SlidingWindowCache<int, int, IntegerFixedStepDomain>(
            adapter, Domain, UserLayerOptions());

        await using var __ = userCache;

        var range = Factories.Range.Closed<int>(300, 310);
