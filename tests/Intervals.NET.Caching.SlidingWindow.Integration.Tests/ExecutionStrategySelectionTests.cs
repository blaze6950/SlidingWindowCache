...
/// <summary>
/// Integration tests verifying the execution strategy selection based on SlidingWindowCacheOptions.RebalanceQueueCapacity.
...
public class ExecutionStrategySelectionTests
{
    ...
    public async Task WindowCache_WithNullCapacity_UsesTaskBasedStrategy()
    {
        ...
        // ACT
        var result = await cache.GetDataAsync(Factories.Range.Closed<int>(10, 20), CancellationToken.None);
    ...
    public async Task WindowCache_WithDefaultParameters_UsesTaskBasedStrategy()
    {
        ...
        // ACT
        var result = await cache.GetDataAsync(Factories.Range.Closed<int>(0, 10), CancellationToken.None);
    ...
    public async Task TaskBasedStrategy_UnderLoad_MaintainsSerialExecution()
    {
        ...
        for (var i = 0; i < 10; i++)
        {
            var start = i * 10;
            var end = start + 10;
            tasks.Add(cache.GetDataAsync(Factories.Range.Closed<int>(start, end), CancellationToken.None).AsTask());
        }
    ...
    public async Task ChannelBasedStrategy_WithCapacityOne_WorksCorrectly()
    {
        ...
        var result1 = await cache.GetDataAsync(Factories.Range.Closed<int>(0, 10), CancellationToken.None);
        var result2 = await cache.GetDataAsync(Factories.Range.Closed<int>(20, 30), CancellationToken.None);
        var result3 = await cache.GetDataAsync(Factories.Range.Closed<int>(40, 50), CancellationToken.None);
    ...
    public async Task TaskBasedStrategy_DisposalCompletesGracefully()
    {
        ...
        await cache.GetDataAsync(Factories.Range.Closed<int>(0, 10), CancellationToken.None);
    ...
    public async Task ChannelBasedStrategy_DisposalDuringActiveRebalance_CompletesGracefully()
    {
        ...
        _ = cache.GetDataAsync(Factories.Range.Closed<int>(0, 10), CancellationToken.None);
