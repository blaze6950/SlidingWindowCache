...
/// <summary>
/// Concurrency and stress stability tests for SlidingWindowCache.
...
public sealed class ConcurrencyStabilityTests : IAsyncDisposable
{
    ...
    {
        return _cache = new SlidingWindowCache<int, int, IntegerFixedStepDomain>(
            _dataSource,
            _domain,
    ...
    public async Task MixedConcurrent_RandomAndSequential_NoConflicts()
    {
        ...
        for (var i = 0; i < totalTasks; i++)
        {
            ...
            if (i % 2 == 0)
            {
                // Sequential
                var start = i * 20;
                range = Factories.Range.Closed<int>(start, start + 30);
            }
            else
            {
                // Random
